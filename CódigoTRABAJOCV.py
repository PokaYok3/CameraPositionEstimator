...
def calibrationFunction():
    ...
    images = glob.glob('mobilePhoneCameraCalibration - copia/calib_*.jpg')
...
def resBundleProjection(Op, x1Data, x2Data, K_c, nPoints):
    ...
    x1proj=K_c@canonical@x_3D_1
    x1proj/=x1proj[2,:]
    x2proj=K_c@canonical@T_21@x_3D_1
    x2proj /= x2proj[2, :]

    #plot2dPoints(x1Data, x1proj, x2Data, x2proj)

    error1=x1proj[0:2,:]-x1Data[0:2,:]
    error1=error1.flatten()
    error2=x2proj[0:2,:]-x2Data[0:2,:]
...
if __name__ == '__main__':
    if True:
        ...
        npzpath='colfotoactual2_colfotoactual1_matches.npz'
        npzpath2 = 'colegiosanagustin1_colfotoactual1_matches.npz'
        ...
        imgMatched = cv2.drawMatches(img_gris_pers_4, cv2.KeyPoint_convert(keypointsSG1), img_gris_pers_5, cv2.KeyPoint_convert(keypointsSG2), dMatchesListG[:1],
        ...
        x3 = np.vstack((srcPts2.T, np.ones((1, srcPts2.shape[0]))))
        print(x3)
        print(x3.shape)
        x4 = np.vstack((dstPts2.T, np.ones((1, dstPts2.shape[0]))))
        print(x4)
        print(x4.shape)
    #Calculo inliers
    if True:
        ...
        indexBestxok1=[]
        indexBestx1=[]
        maskinliers1=np.zeros((x1.shape[1],))

        maskmatches1=np.zeros((x1.shape[1],))
        maskmatchesx1=np.zeros((x1.shape[1],))
        ...
        for k in range(10000):
            ...
            for i in range(4):
                ...
                indexBestx1=np.append(indexBestx1,selection)
                selectionant = selection
                #print(selection)
                x1aux[:,i]=x1[:,selection]
                x2aux[:,i]=x2[:,selection]

            A = np.empty((8, 9))
            j = 0
            for i in range(x1aux.shape[1]):  # In this loop we are taking each point of both images (point matches) and compute the dot prod.
                x1_2d = x1aux[:, i].reshape(3, 1)
                x2_2d = x2aux[:, i].reshape(3, 1)
                A_stack = homographyPts(x1_2d,
                                        x2_2d)  # We perform the right cross prod to generate A matrix for both images.
                A[j:j + 2, :] = A_stack
                j = j + 2
            homography = svd(A)
        ...
        #3) PASAR PUNTOS DE 1 A 2.
            ...
            for i in range(x1.shape[1]):
                distx=pow(x2[0,i]-x2new[0,i],2)
                disty=pow(x2[1,i]-x2new[1,i],2)
                dist=math.sqrt(distx+disty) #dist entre los puntos
                #print(dist)
                #time.sleep(1)


                if(dist<50):#4) COJO DISTANCIAS DE DE LOS NUEVOS Y LOS QUE TENIA  --> MIDO CON DISTANCIA --> VOTACIÓN.
                    votact+=1
                    indexinlier1 = np.append(indexinlier1, i)
            ...
            #time.sleep(5)
            if(votant<votact):
                homographyok=homography
                votant=votact
                indexBestxok1=indexBestx1
                indice=k
                x1ok = x1aux
                x2ok = x2aux
                indexinlierok1 = indexinlier1
            # if(k==10):
        ...
        print(indexinlierok1)
        maskinliers1[indexinlierok1.astype(int)]=1
        maskinliers=maskinliers1.astype(int)
        # mascara para matches.
        maskmatchesx1[indexBestxok1.astype(int)]=1
        maskmatchesx=maskmatchesx1.astype(int)
    ...
    if False:
        ...
        for k in range(10000):
            ...
            for i in range(4):
                ...
                selectionant = selection
                # print(selection)
                x3aux[:, i] = x3[:, selection]
                x4aux[:, i] = x4[:, selection]

            A = np.empty((8, 9))
            j = 0
            for i in range(x3aux.shape[
                               1]):  # In this loop we are taking each point of both images (point matches) and compute the dot prod.
                x1_2d2 = x3aux[:, i].reshape(3, 1)
                x2_2d2 = x4aux[:, i].reshape(3, 1)
                A_stack = homographyPts(x1_2d2,
                                        x2_2d2)  # We perform the right cross prod to generate A matrix for both images.
                A[j:j + 2, :] = A_stack
                j = j + 2
            homography = svd(A)
            homography = homography.reshape(3, 3)
            ...
            # 3) PASAR PUNTOS DE 1 A 2.
            x2new2 = homography @ x3
            x2new2 = x2new2 / x2new2[2, :]
            for i in range(x2new2.shape[1]):
                distx = pow(x4[0, i] - x2new2[0, i], 2)
                disty = pow(x4[1, i] - x2new2[1, i], 2)
                dist = math.sqrt(distx + disty)  # dist entre los puntos
                # print(dist)
                # time.sleep(1)

                if (dist < 5):  # 4) COJO DISTANCIAS DE DE LOS NUEVOS Y LOS QUE TENIA  --> MIDO CON DISTANCIA --> VOTACIÓN.
                    votact += 1
        ...
        # mascara para matches.
        maskmatchesx2[indexBestxok2.astype(int)] = 1
        maskmatchesx2 = maskmatchesx2.astype(int)
        print("Mascara inliers")
        print(maskinliers2)
        a = collections.Counter(maskinliers2)
        print(a)
        print(a[1])
        plt.figure(2)
        plt.title("Set of matches")
        ...
        points3 = np.empty((2, a[1]))
        points4= np.empty((2, a[1]))
    ...
    # Get an estimation of the Fundamental Matrix from point matches

    # SVD [U,S,V] =svd(A) (Af = 0)
    A = []
    for i in range(int(points1.shape[1])):
        A.append(
            [points2[0][i] * points1[0][i], points2[0][i] * points1[1][i], points2[0][i], points2[1][i] * points1[0][i], points2[1][i] * points1[1][i], points2[1][i],
             points1[0][i], points1[1][i], 1])
    ...
    # Since the last eigenvalue of S is already 0, we wouldn't need to enforce rk = 2.
    # However, we do it to show how it should be done.
    s = np.diag(s) # Enforce s to be a diagonal matrix
    s[2, 2] = 0    # We enforce rank 2 by changing s3 in S matrix
    #print("The rank of the S matrix is now: ", np.linalg.matrix_rank(S))
    ...
    # # Getting the 3D points in the same ref
    # x_w23 = np.empty((4, 103))
    # for i in range(x1.shape[1]):
    #     x1_2d = points1[:, i].reshape(2, 1)
    #     x2_2d = points2[:, i].reshape(2, 1)
    #     A23 = rightCrosssP(x1_2d, x2_2d, P_c1_w_new, P_c2_w_new)
    #     point23 = svd(A23, True)
    #     x_w23[:, i] = point23  # Add the vector
    # print("The 3D coordinates of the given matches are: ", '\n', x_w23)
    ...
    #
    # Op = []
    # # Op = [theta[0], theta[1], theta[2], phi, th, points_3dref1[0, i], points_3dref1[1, i], points_3dref1[2, i], 1]
    # Op = [theta[0], theta[1], theta[2], th, phi]
    # for i in range(x_w23.shape[1]):
    #     Op = np.append(Op, x_w23[0, i])
    #     Op = np.append(Op, x_w23[1, i])
    #     Op = np.append(Op, x_w23[2, i])
    #     # Op = np.append(Op, points_3dref1[3, i])
    # print(points1.shape)
    # print(points2.shape)
    # res = resBundleProjection(Op, points1, points2, K_c, nPoints=points1.shape[1])
    # OpOptim = scOptim.least_squares(resBundleProjection, Op, args=(points1,points2,K_c,points1.shape[1]), method='trf')
    # R = sc.linalg.expm(crossMatrix([OpOptim.x[0], OpOptim.x[1], OpOptim.x[2]]))
    # T = [math.sin(OpOptim.x[3]) * math.cos(OpOptim.x[4]), math.sin(OpOptim.x[3]) * math.sin(OpOptim.x[4]),
    #      math.cos(OpOptim.x[3])]
    # T_21 = ensamble_T(R, T)
    # ones = np.ones((1, points1.shape[1]))
    # x_3D_1 = np.empty((4, points1.shape[1]))
    # x_3D_1[3] = ones
    # j = 0
    # for i in range(103):
    #     x_3D_1[0, i] = OpOptim.x[5 + j]
    #     x_3D_1[1, i] = OpOptim.x[6 + j]
    #     x_3D_1[2, i] = OpOptim.x[7 + j]
    #     j += 3
    # T_c2_w_opt = T_21 @ c1_T_w
