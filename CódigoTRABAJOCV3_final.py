...
def resBundleProjection(Op, x1Data, x2Data, Kc, nPoints):
    ...
    error1=x1proj-x1Data
    error1=error1.flatten()
    error2=x2proj-x2Data
...
if __name__ == '__main__':
    ...
    if True:
        ...
        for k in range(20000):
            ...
            for i in range(x4.shape[1]):
                line = F_21 @ x3[:, i]  # l=F@xo linea epipolar

                dist = abs(line[0] * x4[0, i] + line[1] * x4[1, i] + line[2]) / math.sqrt(
                    pow(line[0], 2) + pow(line[1], 2))  # distancia de esa linea al punto x2
                # print(dist)
                # #3
                if (dist < 80):  # 4) COJO DISTANCIAS DE DE LOS NUEVOS Y LOS QUE TENIA  --> MIDO CON DISTANCIA --> VOTACIÓN.
                    votact += 1
        ...
        points3 = np.empty((3, a2[1]))
        points4 = np.empty((3, a2[1]))
        points3[2] = np.ones((1, points3.shape[1]))
        points4[2] = np.ones((1, points4.shape[1]))
        j = 0
    ...
    # Calculo de E  --> 3D and optimization
    if False:
    ...
    if False:
        ...
        # ax.set_ylim(0, 5)
        # ax.set_zlim(0, 5)
        ax.scatter(x_3D_1[0, :], x_3D_1[1, :], x_3D_1[2, :], marker='.', c='red')  # Optimal estimation from solution
    ...
    if False:
        x2dfotref=P_c1@x_3D_1
        x2dfotref/=x2dfotref[2,:]
        x2dfotref = x2dfotref.astype(int)
        print(x2dfotref)
        indexesP=[]
        for i in range(points3aux.shape[1]):
            for j in range(x2dfotref.shape[1]):
                if(points3[0,i]==x2dfotref[0,j]):

                    if (points3aux[1, i] == x2dfotref[1, j]):
                            indexesP = np.append(indexesP, j)
                            print(j)
                            print(x2dfotref[:,j])
